...
def oma_pair(h_near, h_far):
    ...
    rN = bw*np.log2(1 + p*abs(h_near)**2/(CFG.NOISE_W/2))
    rF = bw*np.log2(1 + p*abs(h_far)**2/(CFG.NOISE_W/2))
    return rN, rF
...
def plot_throughput(alts, thr_oma, thr_noma, thr_ris):
    ...
    plt.tight_layout()
    plt.savefig("results/channel_plots/fig3_altitude_throughput.png",dpi=300)
    plt.close()
