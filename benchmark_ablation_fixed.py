...
def run_a2c(hidden=128, n_ep=2000, cfg=BASE):
    ...
    for ep in range(1, n_ep+1):
        state, pos = env.reset()
        states, actions, rewards, values, positions = [], [], [], [], []
        ...
        ep_best_pos = positions[int(np.argmax([r*300 for r in rewards]))]
